...
import { Chip, DataTable, NewRecordButton, PageHeader, Panel } from "@/components/kit";
...
export const Route = createFileRoute("/reservations/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Track plot reservations from hold through expiry and conversion.",
      },
        ...
        <div className="flex items-center gap-1 rounded-lg bg-surface-low p-1">
          <button
            ...
            className={cn(
              ...
              mode === "table" ? "bg-surface-lowest shadow-ambient" : "text-muted-foreground",
            )}
          >
          ...
          <button
            ...
            className={cn(
              "flex h-6 w-6 items-center justify-center rounded-md",
              mode === "board" ? "bg-surface-lowest shadow-ambient" : "text-muted-foreground",
            )}
          >
          ...
          columns={[
            {
              ...
              cell: (r) => (
                <span className="numeric text-xs font-medium">
                  {byId(plots, r.plotId)?.number ?? r.plotId}
                </span>
              ),
            },
            {
              ...
              header: "Customer",
              cell: (r) => (
                <span className="text-xs">{byId(customers, r.customerId)?.name ?? "—"}</span>
              ),
            },
            {
              key: "agent",
            ...
            },
            {
              ...
              align: "right",
              cell: (r) => (
                <span className="numeric text-xs">{formatINR(r.amount, { compact: true })}</span>
              ),
            },
            {
              key: "created",
              header: "Created",
              cell: (r) => (
                <span className="numeric text-xs text-muted-foreground">{r.createdAt}</span>
              ),
            },
            {
              key: "expires",
              header: "Expires",
              cell: (r) => (
                <span className="numeric text-xs text-muted-foreground">{r.expiresAt}</span>
              ),
            },
            {
              key: "countdown",
            ...
            return (
              <Panel key={state} tonal className="p-3">
                ...
                <div className="space-y-2">
                  ...
                  {items.length === 0 && (
                    <p className="px-1 py-4 text-center text-[11px] text-muted-foreground">None</p>
                  )}
                </div>
              </Panel>
            );
