  ...
  head: () => ({
    meta: [
      { title: "Plot Inventory — Bhairava" },
      {
        name: "description",
        content:
          "Spreadsheet-grade plot inventory list with saved views, filters, sorting and inline price edits.",
      },
      { property: "og:title", content: "Plot Inventory — Bhairava" },
      {
        property: "og:description",
        content: "Spreadsheet-grade plot inventory list across every project.",
      },
    ],
  }),
...
type ColumnKey =
  "number" | "project" | "area" | "facing" | "price" | "total" | "status" | "customer" | "agent";
                  ...
                  {(Object.keys(COLUMN_LABELS) as ColumnKey[]).map((c) => (
                    <label key={c} className="flex items-center justify-between gap-2 text-xs">
                      ...
                      <input
                        ...
                        onChange={() => setVisibleCols((prev) => ({ ...prev, [c]: !prev[c] }))}
              ...
              <tr className="bg-surface-low">
                {visibleCols.number && (
                  <th className="px-4 py-3 text-left">
                    <SortHeader col="number" />
                  </th>
                )}
                {visibleCols.project && (
                  <th className="px-4 py-3 text-left">
                    <SortHeader col="project" />
                  </th>
                )}
                {visibleCols.area && (
                ...
                {visibleCols.facing && (
                  <th className="px-4 py-3 text-left">
                    <SortHeader col="facing" />
                  </th>
                )}
                {visibleCols.price && (
                  <th className="px-4 py-3 text-right">
                    <SortHeader col="price" />
                  </th>
                )}
                {visibleCols.total && (
                  <th className="px-4 py-3 text-right">
                    <SortHeader col="total" />
                  </th>
                )}
                {visibleCols.status && (
                  <th className="px-4 py-3 text-left">
                ...
                {visibleCols.customer && (
                  <th className="px-4 py-3 text-left">
                    <SortHeader col="customer" />
                  </th>
                )}
                {visibleCols.agent && (
                  <th className="px-4 py-3 text-left">
                    <SortHeader col="agent" />
                  </th>
                )}
                <th className="w-10" />
              </tr>
            ...
            <tbody>
              {rows.map(({ p, project, customer, agent, price }, i) => (
                <tr
                  key={p.id}
                  className={
                    "transition-colors hover:bg-surface-low " + (i % 2 === 1 ? "bg-surface/60" : "")
                  }
                >
                  {visibleCols.number && (
                    <td className="numeric px-4 py-2.5 font-medium whitespace-nowrap">
                      {p.number}
                    </td>
                  )}
                  {visibleCols.project && (
                    <td className="px-4 py-2.5 whitespace-nowrap">{project}</td>
                  )}
                  {visibleCols.area && (
                  ...
                  {visibleCols.facing && (
                    <td className="px-4 py-2.5 whitespace-nowrap">{p.facing}</td>
                  )}
                  {visibleCols.price && (
                  ...
                  {visibleCols.agent && (
                    <td className="px-4 py-2.5 whitespace-nowrap">{agent || "—"}</td>
                  )}
