...
export const Route = createFileRoute("/payments/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "All incoming payments across bookings, with status, mode and reference." },
      { property: "og:title", content: "Payments — Bhairava" },
      { property: "og:description", content: "All incoming payments across bookings, with status, mode and reference." },
    ],
  }),
        ...
        columns={[
          {
            ...
            header: "Payment",
            cell: (r) => (
              <Link to="/payments/$paymentId" params={{ paymentId: r.id }} className="numeric text-xs font-medium hover:text-primary">
                {r.id}
              </Link>
            ),
          },
          ...
          { key: "reference", header: "Reference", cell: (r) => <span className="numeric text-xs text-muted-foreground">{r.reference}</span> },
          {
            ...
            cell: (r) =>
              r.booking ? (
                <Link to="/bookings/$bookingId" params={{ bookingId: r.booking.id }} className="numeric text-xs text-muted-foreground hover:text-primary">
                  {r.booking.id}
                </Link>
              ) : (
