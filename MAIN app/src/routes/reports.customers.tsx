...
export const Route = createFileRoute("/reports/customers")({
  head: () => ({ meta: [{ title: "Customers Report — Bhairava" }] }),
...
function ReportPage() {
  ...
  const sourceKey = "customers" as const;

  const rows = useMemo(() => {
    const list = ((data as unknown as Record<string, unknown>)[sourceKey] as Array<Record<string, unknown> & { id: string }>) || [];
    return list
      .filter((r) => projectId === "All" || String(r["projectId"] || "") === projectId || sourceKey === "customers")
      .map((r) => ({
        id: r.id,
  ...
  function download() {
    ...
    a.href = url;
    a.download = "report-customers.csv";
    a.click();
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="Customer counts derived from workspace data."
        actions={
          <Btn variant="tonal" onClick={download} data-testid="report-customers-csv">
            Export CSV
          </Btn>
        }
      />
