...
function ProjectStatusSelect({ project }: { project: Project }) {
  ...
  async function onChange(e: ChangeEvent<HTMLSelectElement>) {
    ...
    if (saving || next === project.status) return; // prevent duplicate / no-op writes
    const previous = project.status;
    setSaving(true);
    saveProject({ ...project, status: next }); // optimistic — persisted to the data store
    try {
      // Simulate the status write round-trip so the control shows a saving state.
      await new Promise<void>((resolve) => setTimeout(resolve, 350));
    } catch {
      saveProject({ ...project, status: previous }); // revert on failure
  ...
  return (
    <div className="relative z-10" onClick={(e) => e.stopPropagation()}>
      <select
        ...
        onChange={onChange}
        disabled={saving}
        className="h-9 w-full min-w-[8.5rem] appearance-none rounded-lg bg-surface-low pl-3 pr-8 text-xs font-medium text-foreground outline-none transition-shadow focus:ring-2 focus:ring-primary disabled:opacity-60"
      >
        {options.map((s) => (
      ...
      {saving ? (
        <Loader2 aria-hidden className="pointer-events-none absolute top-1/2 right-2.5 h-3.5 w-3.5 -translate-y-1/2 animate-spin text-muted-foreground" />
      ) : (
        <ChevronDown aria-hidden className="pointer-events-none absolute top-1/2 right-2.5 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
      )}
    </div>
  );
}
...
function ProjectRow({ project, zebra }: { project: Project; zebra: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Status dropdown + resale toggle — above the overlay link so taps don't navigate. */}
      <div className="relative z-10 col-span-2 flex flex-wrap items-center justify-between gap-x-6 gap-y-2 pt-1 lg:col-span-3 lg:justify-end lg:pt-0">
        <div className="flex items-center gap-2">
          <span className="shrink-0 text-[10px] font-semibold tracking-[0.1em] text-muted-foreground uppercase">
            Status
          </span>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">{project.resaleAvailable ? "Available" : "Off"}</span>
