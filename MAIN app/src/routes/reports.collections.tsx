...
import { cashflow, byId, formatINR } from "@/lib/mock-data";
...
export const Route = createFileRoute("/reports/collections")({
  head: () => ({
    meta: [
      { title: "Collections Report — Bhairava" },
      {
        name: "description",
        content:
          "Cashflow, collection performance against target and outstanding balance by project.",
      },
      { property: "og:title", content: "Collections Report — Bhairava" },
      {
...
function CollectionsReport() {
  const { payments, customers, bookings, projects } = useData();
  ...
  const filteredPayments = useMemo(() => {
    const rows =
      projectId === "All"
        ? payments
        : payments.filter((p) => byId(bookings, p.bookingId)?.projectId === projectId);
    return rows.slice(0, 20);
  }, [projectId, payments, bookings]);
    ...
    <AppShell>
      <PageHeader
        ...
        actions={
          <Btn
            ...
            onClick={() => {
              downloadCsv(
                `bhairava-collections-${new Date().toISOString().slice(0, 10)}.csv`,
                ["ID", "Customer", "Amount", "Status", "Date"],
                filteredPayments.map((p) => [
                  p.id,
                  byId(customers, p.customerId)?.name ?? "",
                  String(p.amount),
                  p.status,
                  p.date,
                ]),
              );
              toast.success(`Exported ${filteredPayments.length} payment rows`);
            }}
          >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric
          label="Attainment"
          value={`${attainment}%`}
          delta={attainment >= 100 ? "+on track" : "-behind"}
        />
        <Metric
          label="Outstanding"
          value={`₹${totalOutstanding.toFixed(1)} Cr`}
          hint="latest month"
        />
      </div>
              ...
              <div key={p.name}>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-surface-c">
                  <div
                    ...
                    style={{ width: `${(p.value / maxOutstanding) * 100}%` }}
                  />
                </div>
              </div>
            ))}
          ...
          columns={[
            {
              key: "id",
              header: "Payment",
              cell: (p) => <span className="numeric text-sm font-medium">{p.id}</span>,
            },
            {
              key: "customer",
              header: "Customer",
              cell: (p) => (
                <span className="text-sm">{byId(customers, p.customerId)?.name ?? "—"}</span>
              ),
            },
