...
/** Build a concrete href from a TanStack-style pattern (e.g. "/bookings/$bookingId") + params. */
function buildHref(pattern: string, params?: Record<string, string>): string {
  if (!params) return pattern;
  return pattern.replace(/\$([A-Za-z0-9_]+)/g, (_, key: string) =>
    params[key] != null ? encodeURIComponent(params[key]) : `$${key}`,
  );
}
          ...
          <div key={row.id} className="panel p-4 transition-transform duration-200 active:scale-[0.995]">
            {first && (
              <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                ...
                {linkTo && (
                  <Link
                    href={buildHref(linkTo, params?.(row))}
